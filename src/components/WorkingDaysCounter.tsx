...
const WorkingDaysCounter: React.FC<WorkingDaysCounterProps> = ({
  ...
  useEffect(() => {
    const loadTotal6Months = async () => {
      ...
      // Usamos el valor en vivo del prop para el mes visualizado
      // (más preciso que Firestore y hace que la dependencia sea genuina)
      monthlyCounts[`${year}-${month}`] = clasesDelMesVisible;
    ...
    loadTotal6Months();
  }, [user, clasesDelMesVisible, year, month]);
